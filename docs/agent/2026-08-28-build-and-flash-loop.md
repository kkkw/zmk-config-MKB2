...
- 調査開始時は通常の push と定期実行が `target = all` となり、`build.yaml` の17ターゲットを最大4並列でビルドしていた。その後、定期実行と手動実行を廃止し、push時のみ既定ターゲットをビルドする構成へ変更した。
- 2026-08-29に既定ターゲットを `MKB_L_MODULE_ENC` と `MKB_R_MODULE_TBv4` の2つへ変更した。
...
push時のみ既定の2ターゲットをビルドする。ほかのモジュールに影響する変更では、ローカルビルド手順で全体ビルドを行う。

上流同期で `.github/workflows/` がフォーク元の内容へ戻った場合は、`scripts/restore-push-only-workflows.sh` を実行して、定期実行・手動実行の削除とpush時のみの設定を再適用する。スクリプトが想定外の構造で停止した場合は、上流の変更を確認してから手動で統合する。
